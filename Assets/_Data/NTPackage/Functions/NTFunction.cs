...
namespace NTPackage.Functions
{
    public class NTFunction : NTBehaviour
    {
        ...
        // HashTo6DigitNumber
        public static int HashTo6DigitNumber(string input)
        {
            unchecked
            {
                int hash = 17;
                ...
                return Mathf.Abs(hash) % 1_000_000;
            }
        }
        ...
        public static string Format_Time_Short(long time, int number = 2)
        {
            if (time <= 0) time = 0;
            List<string> list = new List<string>();
            int totalSeconds = (int)time;
            ...
            if (days > 0)
            {
                list.Add(days + Lean.Localization.LeanLocalization.GetTranslationText("d", "d"));
            }
            if (hours > 0)
            {
                list.Add(hours + Lean.Localization.LeanLocalization.GetTranslationText("h", "h"));
            }
            if (minutes > 0)
            {
                list.Add(minutes + Lean.Localization.LeanLocalization.GetTranslationText("m", "m"));
            }
            if (seconds > 0 || list.Count == 0)
            {
                list.Add(seconds + Lean.Localization.LeanLocalization.GetTranslationText("s", "s"));
            }
